...
| D020 | M012 post-validation human selection gate | branding | Canonical icon winner ratification after manual visual review | C2 Nordic Clarity is confirmed as the canonical icon system by human decision | Milestone auto-flow previously advanced using tie-break fallback without explicit human sign-off. User requested reopening visual choice, reviewed the full candidate gallery, and explicitly selected C2. | No | human |
| D021 | M013 discussion | architecture | How app state is represented in URL after cleanup | Use clean URLs for normal app behavior; allow query params only for required OAuth/auth callback mechanics. | User explicitly requested removal of query-param app state coupling while preserving callback protocol requirements. | Yes — if future product requirements need shareable URL state | human |
| D022 | M013 discussion | data-model | Identity contract for planner/runtime/storage | Remove uid-based identity mechanics and align active semantics to userKey + document UUID, while preserving multi-planner capability. | uid was called out as confusing and redundant relative to jsmdma document identity semantics. | No | human |
| D023 | M013 discussion | scope | Treatment of existing share URL/LZ behavior | Remove legacy share surface now and defer replacement sharing semantics to a later milestone decision. | User requested immediate legacy removal and explicit deferral of replacement share design. | Yes — when replacement share requirements are planned | human |
| D024 | M013 discussion | pattern | Preference behavior model for language and theme | Implement user-level language and light/dark preference modes with system live-follow plus explicit override and return-to-system. | User requested system-follow by default unless explicitly chosen otherwise, with live follow when in system mode. | Yes — if UX research indicates different defaults | human |
| D025 | M013 discussion | architecture | Migration strategy for removing legacy contracts | Execute direct cleanup without migration/back-compat scaffolding because project is greenfield with no live users/data. | User confirmed no deployed users/data and explicitly allowed clean-slate removal. | No | human |
| D026 | M013/S02 planning | data-model | How to store and apply system-follow language/theme preferences in S02 without breaking existing preference consumers | Persist explicit preference modes (`system` vs override) alongside effective lang/theme values with normalization fallback for existing numeric preference fields. | S02 must add mode semantics for R107/R108 while preserving compatibility with current runtime code paths that still read numeric preference keys (`1`/`2`). A dual-field normalized contract lets executors add system-follow behavior incrementally and safely. | Yes | agent |
| D027 | M013/S04 planning | verification | How M013 cleanup proof is executed and observed for final closure | Adopt a single orchestrator gate (`scripts/verify-m013-cleanup.sh`) as the canonical local+CI entrypoint, with stage-level JSON report output at `.tests/test-results/m013-cleanup/M013-cleanup-report.json`. | S04 must prove legacy-contract removal end-to-end and close requirement debt without drift between local and CI runs. One entrypoint that runs all grep gates plus smoke/e2e, writes failed-stage context, and is reused by CI gives deterministic proof and fast failure localization for R007/R109 closure. | Yes | agent |
